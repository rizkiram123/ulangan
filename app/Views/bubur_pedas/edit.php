...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
        <div class="bg-primary px-8 py-6 text-white flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-extrabold">Edit Menu Bubur Pedas</h1>
                <p class="text-xs text-indigo-100 mt-1">Perbarui informasi menu #<?= esc($menu['id']) ?></p>
            </div>
        ...
        <form action="<?= base_url('/buburpedas/update/' . $menu['id']) ?>" method="post" class="p-8 space-y-6">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div class="sm:col-span-2 space-y-2">
                    ...
                    <input type="text" id="nama" name="nama" value="<?= old('nama', $menu['nama']) ?>" placeholder="Nama menu..." class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm transition" required>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select id="kategori" name="kategori" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm transition font-semibold">
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= $cat ?>" <?= old('kategori', $menu['kategori']) === $cat ? 'selected' : '' ?>><?= $cat ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="harga" name="harga" value="<?= old('harga', $menu['harga']) ?>" placeholder="25000" class="w-full pl-12 pr-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm transition" min="0" required>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" id="stok" name="stok" value="<?= old('stok', $menu['stok']) ?>" placeholder="10" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm transition" min="0" required>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="url" id="gambar" name="gambar" value="<?= old('gambar', $menu['gambar']) ?>" placeholder="https://images.unsplash.com/..." class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm transition">
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="3" placeholder="Penjelasan menu..." class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm transition"><?= old('deskripsi', $menu['deskripsi']) ?></textarea>
            </div>
            ...
            <div class="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
                ...
                <button type="submit" class="bg-primary hover:bg-primary-hover text-white font-bold px-7 py-3 rounded-xl shadow-lg shadow-primary/30 transition text-sm flex items-center gap-2">
                    <i class="fa-solid fa-arrows-rotate"></i> Update Menu
                </button>
            </div>
        </form>
    </div>
